...
var MainThreadAnimation = class extends BaseAnimation {
  ...
  tick(timestamp, sample = false) {
    const { generator, mirroredGenerator, mapPercentToKeyframes, keyframes: keyframes3, calculatedDuration, totalDuration, resolvedDuration } = this.resolved;
...
var AcceleratedAnimation = class extends BaseAnimation {
  constructor(options) {
    super(options);
    const { name, motionValue: motionValue2, keyframes: keyframes3 } = this.options;
    this.resolver = new DOMKeyframesResolver(keyframes3, (resolvedKeyframes) => this.onKeyframesResolved(resolvedKeyframes), name, motionValue2);
    this.resolver.scheduleResolve();
  }
  initPlayback(keyframes3) {
    ...
    const animation = animateStyle(motionValue2.owner.current, name, keyframes3, { ...this.options, duration });
    ...
    if (this.pendingTimeline) {
      ...
      animation.onfinish = () => {
        const { onComplete } = this.options;
        motionValue2.set(getFinalKeyframe(keyframes3, this.options));
        onComplete && onComplete();
        this.cancel();
        this.resolveFinishedPromise();
        this.updateFinishedPromise();
  ...
  stop() {
    this.isStopped = true;
    const { animation, keyframes: keyframes3 } = this.resolved;
